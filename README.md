...
**Статус:** рабочая локальная версия с тремя кабинетами — сотрудник, руководитель отдела и HR. Реализованы маршрут сотрудника, цель, матрица навыков, рекомендации по правилам, «Моё обучение», сводка по компании или отделу, очередь поддержки «Нужна поддержка / Готовы к росту», карточка сотрудника, создание профиля, импорт JSON/CSV и подтверждение завершения с пересчётом. GitHub и Jira подключаются самим сотрудником с согласием и работают на синтетических демо-данных; GitHub есть только у тех, у кого он есть. AI подключён через OpenAI (`gpt-6-sol`) в двух местах: объяснение рекомендаций сотруднику и «AI-разбор для разговора» в карточке сотрудника для HR и руководителя; живые вызовы проверены (4–7 с). Живые интеграции, редактор обучений/тестов, первичная оценка и награды — следующие этапы.
...
Выберите «Сотрудник», «Руководитель» или «HR-партнёр». Начальный пароль демо-аккаунтов: `careerquest-demo`. Логины API: `employee` (привязан к `E0002`), `manager` (руководитель `E0050`, видит только свой отдел Backend Development) и `hr`. Учётки и демо-подключения создаются при старте и для уже существующей базы. Вход нового добавленного профиля пока не создаётся автоматически; его видит HR. `DEMO_PASSWORD` задаёт пароль при первом создании базы, а не меняет существующие пароли.
...
AI вызывается только кнопками «Объяснить с AI» (кабинет сотрудника) и «Подготовить с AI» (карточка сотрудника у HR и руководителя), а не при открытии страниц. Разбор для разговора получает только факты карточки без имени, ID, отдела и руководителя; ссылка на мероприятие сверяется со списком проверенных кандидатов. Сервер сначала выбирает допустимые курсы, модель объясняет их; числовые оценки и права не передаются модели на усмотрение. В запросе нет имени, ID сотрудника и сырых рабочих документов. Ответ ограничен схемой; идентификаторы сверяются с кандидатами. Включены `store=false`, кэш по фактам/модели/версии промпта, ограничение 20 вызовов в UTC-день и тайм-аут 9 секунд. Ошибка или отсутствие ключа дают явно подписанный ответ по правилам. `store=false` не означает автоматическое отсутствие всех провайдерских журналов; корпоративные данные требуют отдельного согласования.
...
## Кабинеты и аналитика

| Кабинет | Что видит |
| --- | --- |
| Сотрудник | Мой маршрут (рейтинг развития = готовность к своей цели, рекомендации, требования грейда), Моё обучение, Подключения GitHub/Jira |
| Руководитель | Сводку и очередь поддержки только своего отдела; профили — только чтение |
| HR | Сводку по компании с фильтрами, очередь поддержки, карточки, импорт, создание профилей, подтверждение завершений |

Очередь «Нужна поддержка»: `100 × (0,30·К + 0,20·Д + 0,20·С + 0,15·В + 0,15·Ц)` — ключевые навыки ниже текущего грейда, покрытие текущего грейда (К и Д умножаются на стаж/12), нет добровольного обучения за 6 месяцев, пропусков больше, чем завершений, нет цели при стаже от года. Высокий — от 50, средний — от 40. «Готовы к росту» — от 65 баллов по готовности к цели, ключевым навыкам, приросту за год, запасу по грейду и вовлечённости. Веса и пороги — в `backend/app/analytics.py`; просроченное обязательное обучение показывается отдельно и в балл не входит. Это очередь поддержки для HR и руководителя, а не публичный рейтинг.

GitHub и Jira (`backend/app/integrations.py`) — детерминированные синтетические данные, поэтому работают и для профилей жюри. Jira доступна всем; GitHub — если в профиле есть необязательное поле `github_login` или по синтетическому правилу для инженеров, QA и части аналитиков. Подключает только сам сотрудник, с согласием; HR видит выжимку лишь подключённых источников. Выжимка — гипотеза для разговора, уровни навыков не меняет.
...
Источники, критерии по профессиям, учёт времени, права, проверка выводов, ограничения и последовательность подключения описаны в [спецификации интеграций](docs/WORK_EVIDENCE.md). Это проектирование: живых подключений пока нет. Для расширения потребуется фоновый Python worker; первоначальная очередь заданий — в PostgreSQL. Рекомендации выдаются по подготовленным результатам, полный анализ рабочих материалов выполняется асинхронно.
...
- [x] Реальный вызов OpenAI: объяснение рекомендаций и AI-разбор для HR/руководителя.
- [ ] Систематическая оценка качества AI-обоснований на наборе профилей.
...
- [ ] Анализ синтетических рабочих примеров разработчика и бизнес-аналитика с проверкой экспертом.
- [x] Кабинет руководителя отдела, сводка по компании, очередь поддержки и готовность к росту.
- [x] Демо-подключения GitHub и Jira на синтетических данных с согласием сотрудника.
- [ ] Живые подключения GitHub, Jira и Confluence для согласованных тестовых проектов; дальнейшие адаптеры по мере необходимости.
