...
**Статус:** рабочая локальная версия. Реализованы кабинет сотрудника, цель от HR, матрица навыков, рекомендации по правилам, история, HR-обзор, добавление сотрудника с отдельным входом по желанию HR, фиксация первичной оценки с основаниями, импорт JSON/CSV и подтверждение завершения с пересчётом. OpenAI подключён локально: один реальный запрос на синтетическом профиле вернул объяснения для трёх курсов; полная оценка качества и задержек ещё впереди. Автоматизированное интервью/тестирование, повторная аттестация, интеграции, редактор обучений/тестов и награды — следующие этапы, не готовые функции.

## Приложение и макет команды

В репозитории объединены рабочее приложение и материалы ветки `docs/analytics-and-plan`. Это два отдельных интерфейса, а не одна сборка:

| Вариант | Что доступно | Где находится |
| --- | --- | --- |
| Рабочее приложение | Сервер, база данных, роли сотрудника и HR, первичная оценка, цель только от HR, рекомендации и подтверждения | `backend/`, `frontend/`; запуск на порту 8000 |
...
Кабинет руководителя и новые экраны макета ещё не перенесены в рабочее приложение. Переключение ролей в макете не защищает данные; используйте только синтетический набор. Живых интеграций GitHub/Jira/Confluence нет ни в одном варианте.
...
| AI | PydanticAI + OpenAI Responses API; стартовая модель `gpt-6-sol` | Объяснение выбранных сервером шагов через Structured Outputs; один реальный вызов на синтетическом профиле проверен |
...
- [ ] Оценка качества AI-обоснований на наборе профилей.
