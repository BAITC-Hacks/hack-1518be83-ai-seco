...
**Статус:** первая рабочая локальная версия. Реализованы кабинет сотрудника, цель, матрица навыков, рекомендации по правилам, история, HR-обзор, создание профиля, импорт JSON/CSV и подтверждение завершения с пересчётом. AI-адаптер написан, но реальный OpenAI-вызов ещё не проверен: нужен ключ проекта. Интеграции, редактор обучений/тестов, первичная оценка и награды — следующие этапы, не готовые функции.
...
Выберите «Сотрудник» или «HR-партнёр». Начальный пароль обоих демо-аккаунтов: `careerquest-demo`. Логины API: `employee` и `hr`; сотрудник привязан к `E0002`. Вход нового добавленного профиля пока не создаётся автоматически; его видит HR. `DEMO_PASSWORD` задаёт пароль при первом создании базы, а не меняет существующие пароли.
