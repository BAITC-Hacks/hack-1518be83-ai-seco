...
**Статус:** рабочая локальная версия с тремя ролями: сотрудник, HR и руководитель своего отдела. Есть цель только от HR, матрица навыков, рекомендации, отдельные AI-объяснения, «Моё обучение», очередь поддержки, планы и паузы с ответственным и датой встречи, настраиваемые сигналы, уточнение даты грейда с историей. Сохранены добавление сотрудника, первичная оценка с основаниями, импорт JSON/CSV и подтверждение завершения с пересчётом. OpenAI подключён локально и проверен на синтетическом профиле; полноценная оценка качества и задержек ещё впереди. Автоматизированное тестирование сотрудников, повторная аттестация, живые интеграции, редактор обучений/тестов и награды — следующие этапы. [Текущее состояние и очередь работ](docs/IMPLEMENTATION_PLAN.md).
...
В приложении права руководителя ограничены явно назначенным на сервере отделом; без назначения доступ закрыт. В исходном макете переключение ролей не защищает данные. Живых интеграций GitHub/Jira/Confluence нет ни в одном варианте; используйте только синтетический набор.
...
Статус: планы, сроки, паузы и общие HR-пороги реализованы. Дополнительный каталог общих курсов, предпочтения и награды остаются в плане.
...
- [ ] Измерение задержек и оценка качества живых AI-объяснений на наборе профилей.
